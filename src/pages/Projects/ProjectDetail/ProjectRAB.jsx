...
// Data RAB Diubah menjadi format Hierarki (Grouped per Divisi)
...
export default function ProjectRAB({ selectedProject }) {
  ...
  const renderValue = (val, defaultColor = 'text-slate-300') => {
    ...
    if (val.includes('#ERROR!')) return <span className="text-rose-400 font-bold bg-rose-950/40 px-1.5 py-0.5 rounded text-[9px]">{val}</span>;
    if (val.includes('#REF!')) return <span className="text-amber-400 font-bold bg-amber-950/40 px-1.5 py-0.5 rounded text-[9px]">{val}</span>;
    ...
    <div className="w-full space-y-5">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 shrink-0 mb-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full xl:w-auto overflow-x-auto pb-1 sm:pb-0">
          ...
          {/* Card 1: Action Bar (Edit Mode & Export/Import) */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4">
          ...
          <p className="text-lg font-bold text-white mt-1">{formatRupiah(project.nilaiKontrak)}</p>
        </div>

        <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4">
          <span className="text-slate-400 text-[11px] font-medium flex items-center gap-1.5">
            <PieChart className="w-4 h-4 text-emerald-400" /> Realisasi Anggaran Terkini
          </span>
          <p className="text-lg font-bold text-emerald-400 mt-1">{formatRupiah(project.nilaiKontrak * (progress / 100))}</p>
        </div>

        <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4 flex flex-col justify-center">
          <div className="flex justify-between items-center mb-2">
            <span className="text-slate-400 text-[11px] font-medium flex items-center gap-1.5">
              <TrendingDown className="w-4 h-4 text-blue-400" /> Serapan Biaya
            </span>
      ...
      {/* Render Data RAB per Divisi (Card Based Layout) */}
      <div className="space-y-6">
        {groupedRABData.map((divisi) => (
          <div key={divisi.id} className="bg-slate-800/60 border border-slate-700/60 rounded-2xl overflow-hidden shadow-lg">
            
            {/* Card Header Divisi */}
            <div className="bg-slate-900/80 px-5 py-4 border-b border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <h3 className="text-sm font-extrabold text-amber-400 tracking-wide uppercase">
                {divisi.namaDivisi}
              </h3>
              {/* Tombol aksi divisi hanya muncul jika Edit Mode aktif */}
              {isEditMode && (
                <div className="flex items-center gap-2 animate-fade-in">
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-blue-400 hover:text-blue-300 text-[10px] font-semibold rounded-lg border border-slate-700/80 transition-all">
                    <ListPlus className="w-3.5 h-3.5" /> Tambah Item
                  </button>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-[10px] font-semibold rounded-lg border border-slate-700/80 transition-all">
                    <Edit3 className="w-3.5 h-3.5" /> Edit Divisi
                  </button>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-[10px] font-semibold rounded-lg border border-rose-500/20 transition-all">
                    <Trash2 className="w-3.5 h-3.5" /> Hapus Divisi
                  </button>
                </div>
              )}
            </div>

            {/* Inner Table per Divisi */}
            <div className="overflow-x-auto">
              <table className="w-full table-fixed text-left border-collapse">
                <thead className="bg-slate-900/40 text-[10px] uppercase text-slate-400 border-b border-slate-700/50">
                  <tr>
                    <th className="px-4 py-2.5 w-[35%] border-r border-slate-700/40 font-semibold">Uraian Pekerjaan</th>
                    <th className="px-2 py-2.5 w-[6%] border-r border-slate-700/40 text-center font-semibold">SAT</th>
                    
                    {/* Header Group Rencana */}
                    <th className="px-2 py-2.5 w-[7%] border-r border-slate-700/40 text-right font-semibold text-blue-400/80 bg-blue-950/10">Vol (R)</th>
                    <th className="px-3 py-2.5 w-[14%] border-r border-slate-700/40 text-right font-semibold text-blue-400/80 bg-blue-950/10">Harga Sat (R)</th>
                    <th className="px-3 py-2.5 w-[14%] border-r border-slate-700/40 text-right font-semibold text-blue-400/80 bg-blue-950/10">Jumlah (R)</th>

                    {/* Header Group Realisasi */}
                    <th className="px-2 py-2.5 w-[7%] border-r border-slate-700/40 text-right font-semibold text-emerald-400/80 bg-emerald-950/10">Vol (A)</th>
                    <th className="px-3 py-2.5 w-[17%] text-right font-semibold text-emerald-400/80 bg-emerald-950/10">Jumlah Realisasi (A)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-700/30 text-[11px] text-slate-300">
                  {divisi.pekerjaan.map((item) => {
                    if (item.isSubheader) {
                      return (
                        <tr key={item.id} className="bg-slate-800/40">
                          <td colSpan={7} className="px-4 py-2 font-semibold text-slate-200">
                            {item.label}
                          </td>
                        </tr>
                      );
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-700/20 transition-colors group">
                        <td className="px-4 py-2.5 border-r border-slate-700/40 group-hover:text-white transition-colors" title={item.label}>
                        ...
                        <td className="px-2 py-2.5 text-center border-r border-slate-700/40 text-slate-400 font-mono text-[10px]">{item.sat}</td>
                        
                        {/* Rencana Data */}
                        <td className="px-2 py-2.5 text-right border-r border-slate-700/40 font-mono text-[10px] bg-blue-950/5">{renderValue(item.renVol)}</td>
                        <td className="px-3 py-2.5 text-right border-r border-slate-700/40 font-mono text-[10px] bg-blue-950/5">{renderValue(item.renHarga)}</td>
                        <td className="px-3 py-2.5 text-right border-r border-slate-700/40 font-mono text-[10px] bg-blue-950/5">{renderValue(item.renJumlah, 'text-blue-300 font-medium')}</td>

                        {/* Realisasi Data */}
                        <td className="px-2 py-2.5 text-right border-r border-slate-700/40 font-mono text-[10px] bg-emerald-950/5">{renderValue(item.realVol)}</td>
                        <td className="px-3 py-2.5 text-right font-mono text-[10px] bg-emerald-950/5">{renderValue(item.realJumlah, 'text-emerald-400 font-medium')}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="bg-slate-900 px-4 py-3 border-t border-slate-700/80 flex flex-col md:flex-row md:items-center justify-end gap-4 text-xs">
              <span className="font-bold text-slate-400 uppercase">Subtotal {divisi.namaDivisi}</span>
              <div className="flex items-center gap-6 font-mono font-bold">
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-blue-400 text-sm">Rp {divisi.totalRencana}</span>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-[9px] text-slate-500 uppercase">Realisasi (Actual)</span>
                  <span className="text-emerald-400 text-sm">Rp {divisi.totalRealisasi}</span>
                </div>
              </div>
            </div>
