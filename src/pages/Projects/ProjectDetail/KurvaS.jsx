  ...
  return (
    <div className="w-full space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 bg-slate-800/60 border border-slate-700/60 p-5 rounded-2xl flex flex-col shadow-lg">
          <div className="flex items-center justify-between border-b border-slate-700/60 pb-3 mb-4">
            <div className="flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-amber-500" />
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">Grafik Kumulatif Rencana vs Realisasi</h3>
            </div>
          ...
          <div className="w-full h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dummyKurvaS} margin={{ top: 10, right: 15, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="minggu" stroke="#94a3b8" fontSize={11} tickLine={false} />
                <YAxis stroke="#94a3b8" fontSize={11} domain={[0, 100]} unit="%" tickLine={false} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '10px', fontSize: '12px', padding: '8px 12px' }}
                />
        ...
        <div className="lg:col-span-5 bg-slate-800/60 border border-slate-700/60 rounded-2xl flex flex-col overflow-hidden shadow-lg">
          <div className="px-5 py-4 border-b border-slate-700/60 flex items-center justify-between bg-slate-900/50">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <PieChart className="w-4 h-4 text-amber-500" /> Rincian Progress Pekerjaan Utama
            </h3>
            <span className="text-[10px] text-slate-400 bg-slate-800 px-2.5 py-0.5 rounded-md border border-slate-700 font-mono">
          ...
          <div className="flex-1 overflow-y-auto max-h-[350px]">
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="text-[10px] uppercase text-slate-400 bg-slate-900/80 sticky top-0 z-10 border-b border-slate-700/80">
                <tr>
              ...
              <tbody className="divide-y divide-slate-700/40 text-xs">
                {dummyItemProgress.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-700/30 transition-colors">
                    <td className="px-3 py-2.5 text-slate-200 truncate" title={item.nama}>
                      <span className="text-amber-500 font-mono text-[10px] mr-1">#{item.id}</span>
                      {item.nama}
                    </td>
                    <td className="px-2 py-2.5 text-center text-slate-400 font-mono text-[10px] truncate">
                      {item.volume} {item.satuan}
                    </td>
                    <td className="px-2 py-2.5 text-center text-slate-400 font-mono text-[10px]">
                      {item.bobot}%
                    </td>
                    <td className="px-3 py-2.5 text-right">
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[10px] font-bold text-amber-400 font-mono">{item.progress}%</span>
      ...
      <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl overflow-hidden shadow-lg">
        <div className="px-5 py-4 border-b border-slate-700/60 bg-slate-900/50 flex items-center justify-between">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
            <FileSpreadsheet className="w-4 h-4 text-amber-500" /> Tabel Parameter Kumulatif Mingguan (S-Curve Table)
          </h3>
          <span className="text-[11px] text-slate-400 font-mono">Periode Pelaksanaan Kontrak</span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/80 border-b border-slate-700/60 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                ...
                <th className="p-3 text-right bg-blue-950/20 text-blue-300">Kumulatif Rencana (%)</th>
                <th className="p-3 text-right bg-emerald-950/20 text-emerald-300">Kumulatif Realisasi (%)</th>
                <th className="p-3 text-center">Deviasi (%)</th>
                <th className="p-3 text-center">Status Evaluasi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/50 text-xs text-slate-300 font-mono">
              {dummyKurvaS.map((row, idx) => {
                const isDevNegative = row.deviasi < 0;

                return (
                  <tr key={idx} className="hover:bg-slate-700/30 transition-all">
                    <td className="p-3 text-center font-bold text-white">{row.minggu}</td>
                    <td className="p-3 text-right text-slate-300">{row.bobotRencana.toFixed(2)}%</td>
                    <td className="p-3 text-right text-slate-300">{row.bobotRealisasi.toFixed(2)}%</td>
                    <td className="p-3 text-right text-blue-400 font-semibold bg-blue-950/5">{row.rencanaKumulatif.toFixed(2)}%</td>
                    <td className="p-3 text-right text-emerald-400 font-semibold bg-emerald-950/5">{row.realisasiKumulatif.toFixed(2)}%</td>
                    <td className={`p-3 text-center font-bold ${isDevNegative ? 'text-rose-400' : 'text-amber-400'}`}>
                      {row.deviasi > 0 ? `+${row.deviasi.toFixed(2)}` : row.deviasi.toFixed(2)}%
                    </td>
                    <td className="p-3 text-center">
                      {isDevNegative ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-sans font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20">
                          <AlertTriangle className="w-3 h-3" /> Terlambat
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-sans font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
